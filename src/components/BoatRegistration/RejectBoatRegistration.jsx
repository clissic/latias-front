...
export function RejectBoatRegistration() {
  const { id } = useParams();
  ...
  const handleConfirm = async () => {
    try {
      setConfirming(true);
      const response = await fetch(`/api/boats/registration/reject/${id}?token=${token}`);
      ...
      if (data.status === "success") {
        Swal.fire({
          ...
          text: "El registro del barco ha sido rechazado y eliminado del sistema.",
          confirmButtonText: "Aceptar",
          background: "#082b55",
          color: "#ffffff",
          customClass: {
        ...
        throw new Error(data.msg || "Error al rechazar el registro");
      }
    } catch (error) {
      Swal.fire({
        icon: "error",
        title: "Error",
        text: error.message || "Error al rechazar el registro",
        confirmButtonText: "Aceptar",
        background: "#082b55",
        color: "#ffffff",
        customClass: {
          ...
          <div className="col-12 col-md-10 col-lg-10 col-xl-10">
            <div className="flota-form-container">
              <div className="d-flex justify-content-center align-items-center mb-3">
                <h5 className="text-white mb-0 px-3 py-2 rounded" style={{ backgroundColor: '#dc3545', textTransform: 'uppercase' }}>Confirmar Rechazo de Registro</h5>
              </div>
              <div className="div-border-color my-3"></div>
              <div className="text-white">
                <p className="text-center mb-4">
                  ¿Estás seguro de que deseas <strong>RECHAZAR</strong> el registro del siguiente barco?
                </p>
                <p className="text-warning text-center mb-4">
                  <strong>Advertencia:</strong> Esta acción eliminará permanentemente el registro del barco del sistema.
                </p>
